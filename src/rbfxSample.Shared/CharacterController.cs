...
namespace rbfxSample
{
    ...
    public class CharacterController: LogicComponent
    {
        ...
        public override void FixedUpdate(float timeStep)
        {
            ...
            if (rigidBody_ == null)
                rigidBody_ = Node.GetComponent<RigidBody>(false);
            ...
            else
            {
                ...
                {
                    rigidBody_.Position = ((rigidBody_.Position - (newPos - pos) * 2.0f));
                }
                else if (nearGround_)
                {
                    rigidBody_.Position = ((rigidBody_.Position - (newPos - pos) * 3.0f));
                }
            ...
            if (highImpulseDetected_ && nearGround_)
            {
                ...
                rigidBody_.LinearVelocity = new Vector3(vel.X, Math.Min(vel.Y, 0.0f), vel.Z);
            }
            ...
            if (softGrounded && !highImpulseDetected_)
            {
                if (controls_.IsDown(CTRL_JUMP) && okToJump_)
                {
                    ...
                    rigidBody_.LinearVelocity = (new Vector3(vel.X, Math.Max(vel.Y, 0.0f), vel.Z));
